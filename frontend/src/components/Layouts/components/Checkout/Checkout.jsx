...
const Checkout = () => {
  ...
  const { selectedItems = [], totalAmount = 0 } = state || {};
  const displayItems = state?.selectedItems?.length > 0 ? state.selectedItems : [];
  ...
  useEffect(() => {
    ...
    if (!savedData) {
      message.warning('Một số sản phẩm trong giỏ hàng vừa được cập nhật, bạn vui lòng kiểm tra và thử lại.');
      navigate('/cart');
      return;
    }
  }, [location]);
  ...
  useEffect(() => {
    const fetchAddresses = async () => {
      try {
        ...
        // Nếu API trả về object có thuộc tính data là array
        const addressArr = Array.isArray(res) ? res : Array.isArray(res?.data) ? res.data : [];
        setAddresses(addressArr);
        const defaultAddress = addressArr.find((addr) => addr.isDefault);
        if (defaultAddress) {
    ...
    try {
      if (values.isDefault) {
        const updates = addresses
          ...
          .map((addr) => updateAddress(addr.id, { ...addr, isDefault: false }));
  ...
  return (
    <div className="mx-auto mt-16 max-w-7xl px-8 py-4">
      ...
      {displayItems.length === 0 ? (
        ...
        <div className="flex flex-col gap-6 lg:flex-row">
          <div className="flex-1">
            ...
            <OrderSummary totalAmount={totalAmount} handleOrder={handleOrder} loading={loading} />
          </div>
        </div>
      )}
